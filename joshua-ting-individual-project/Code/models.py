...
def run_modeling_wrapper(df_train, df_test, df_val, ss_target, df_test_untouched, random_seed = 33, run_base_estimators = False, run_model_tuning = False, fast_gridsearch = True, save_model = False, demo = False):
    '''
    Wrapper to run all modeling processes.

    Parameters
        train_df: training dataframe
        test_df: testing dataframe
        df_val: validation dataframe
        ss_target: standard scaler for target label
        df_test_untouched: test dataframe without preprocessing to keep human readable features for results
        random_seed: seed to set for pseudorandom objects, default 33
        run_base_estimators: set to True to run base model comparison
        run_model_tuning: set to True to run entire model tuning and gridsearch (takes a couple of hours of runtime to iterate over each hyperparameter)
        fast_gridsearch: set to True to run a much smaller set of hyperparameters in gridsearch
        save_model: set to True to save results from modeling phase
        demo: set to True to skip all modeling phase

    Return
        None

    '''
    # If demo, just skip to end
    if demo:
        print('Is a demo, skipping time intensive modeling section and just loading results to GUI.')
    else:
        ####################################################
